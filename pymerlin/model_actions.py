...
def spawn(model, child):
    ...
    :return:
    """
    topics = get_topics(model._model_type, child.definition) if child.definition.type == "ACTIVITY" else (None, None)
...
def call(model, child):
    if type(child) is TaskSpecification:
        task_provider = lambda: activity_wrapper(
            child,
            child.args,
            model,
            *get_topics(model._model_type, child.definition))
        return _yield_with(pymerlin._internal._task_status.Calling(task_provider))
    else:
        return _yield_with(pymerlin._internal._task_status.Calling(lambda: child))
