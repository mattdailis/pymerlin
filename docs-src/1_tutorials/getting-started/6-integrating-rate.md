...
will need to be a `MutableResource`. The declaration looks just like `recording_rate`
...
Let's begin by creating one more discrete `MutableResource` called `ssr_volume_upon_rate_change` in our `DataModel`
...
to `recording_rate`. Since this resource is more of a "helper" resource and doesn't need to be exposed to our planners,
